...
if(isEditing){
  try {
    await axios.put(`http://localhost:3000/dispositivos/${editId}`, form);
    setDispositivos( dispositivos.map((d)=>d._id === editId? {...d , ...form}: d));
    setIsEditing(false);
    setEditId(null);
  } catch (error) {
  ...
  try {
    const response = await axios.post('http://localhost:3000/dispositivos', form);
  ...
  function handleEdit(id){
    ...
    setForm({ip:dispositivo.ip, nome:dispositivo.nome , descricao:dispositivo.descricao});
    setIsEditing(true);
    setEditId(id);
  }
...
async function handleDelete(id){
  const isConfirmed = window.confirm('Tem certeza que quer deletar o dispositivo?');
  if(isConfirmed){
    try {
...
<button type="submit" className="btn btn-primary">{isEditing?'Atualizar':'Cadastrar'}</button>
  <button type="reset" className="btn btn-secondary" onClick={()=>{setForm({ip: '', nome :'' , descricao:''})}}>Limpar</button>
</div>
        ...
        (dispositivo)=>{
          return(
            <tr key={dispositivo._id}>
              ...
              <td>
                <button className='btn btn-warning me-2' onClick={()=>{handleEdit(dispositivo._id)}}>Editar</button>
